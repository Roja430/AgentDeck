...
- **Windows** — per-user **Scheduled Task** `AgentDeckDaemon` with a **logon trigger**, registered via built-in `schtasks.exe` (no npm dependency, no admin elevation). `agentdeck daemon install` registers + immediately `/Run`s it and installs Codex hooks; `uninstall` gracefully `/shutdown`s the daemon then `/Delete`s the task. Builder + schtasks wrappers live in `bridge/src/windows-service.ts`; the pure XML builder is unit-tested in `bridge/src/__tests__/windows-service.test.ts` (the schtasks calls are integration-only).
  ...
  - Task settings mirror the LaunchAgent: `LogonType=InteractiveToken` + `RunLevel=LeastPrivilege` (no elevation), `MultipleInstancesPolicy=IgnoreNew` (the singleton guard already dedupes), `RestartOnFailure` Interval=PT1M/Count=3 (≈KeepAlive), `ExecutionTimeLimit=PT0S`, no stop on idle/battery.
  - **The task launches `wscript.exe`, not `node.exe`.** node is a console-subsystem binary, so pointing the task straight at it popped a console window on every logon. `<Hidden>` does not fix this — it hides the task in the Task Scheduler UI, not the process's window. `agentdeck daemon install` therefore generates two shims in `~/.agentdeck/`:
    - `daemon-autostart.vbs` — `WshShell.Run(cmd, 0, True)`. wscript is GUI-subsystem so nothing flashes, and `0` (SW_HIDE) is the supported way to start a console program with no console. It **waits** and forwards the exit code, so the task still tracks the live daemon and `RestartOnFailure` keeps supervising it; running detached would report success the instant the daemon started.
    - `daemon-autostart.cmd` — runs `node.exe "<cli.js>" daemon start --foreground` with output redirected to `~/.agentdeck/daemon-autostart.log`. A hidden console discards stdout, so **that log file is the only place autostart output goes** — read it first when the daemon does not come up after a reboot. It truncates per logon (one file = one boot) rather than appending, so it cannot grow without bound. `chcp 65001` is on line 1 because cmd.exe decodes the batch file with the console codepage and a non-ASCII profile path would otherwise be mangled. The `.vbs` is deliberately pure ASCII for the same reason.
    - Both are regenerated by `daemon install` and removed by `daemon uninstall`; deleting them by hand just means reinstalling.
